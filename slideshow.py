...
class SlideShow(ui.View):
    ...
    def show_and_hide_menu(self, data):
        ...
        find_scripter_instance().pause_play_all()
    ...
    def tableview_cell_for_row(self, tableview, section, row):
        ...
        def choose_symbol():
            return (self.symbol_active
                if self.album_list[row].get('active', True)
                else self.symbol_inactive)
        
        def toggle_active(sender):
            self.album_list[row]['active'] = not self.album_list[row].get('active', True)
